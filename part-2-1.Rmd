...
yrblt <- ggplot(oo_yrblt,
  ...
  facet_wrap(~county) +
  labs(title = "Owner-occupied housing by year built", subtitle = "Total housing units in 2020", fill = "Year built") +
  theme(axis.title.x = element_blank(), 
        axis.title.y = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
```
The majority of existing homeowner housing across the region is and has been three- or more bedroom homes since 2016. Smaller housing options exist largely in the City of Richmond or Henrico County. While one- to two-bedroom housing is often much more affordable, these housing options are often in older, but highly desirable neighborhoods in the City of Richmond (i.e. The Fan and Church Hill).
```{r oo-bedrooms}
...
Chesterfield County continued to lead the region in home sales --- hitting a monthly peak in June 2021, with a total of 809 sales. In nearly all localities except for Chesterfield County, the average monthly home sales has largely remained the same. Only in Chesterfield County was there a more than 10 percent increase (15 percent increase) in average monthly home sales between 2019 and 2021.
...
The affordability of resale homes compared to new construction has often made them the first rung on the homeownership ladder. But since the start of the pandemic, the median resale home price has risen above the \$300,000 mark and in June 2022 reached a high of \$371,000. During this timeframe, new construction median home prices have remained above \$350,000 and throughout 2022 so far have stayed above \$400,000. On average, there is a \$89,127 difference between new construction and resale sales price --- leaving new construction significantly out of reach for lower income households.
...
price_comp_bar <- ggplot(pha_price_comp,
...
br_comp_bar <- ggplot(pha_br_comp,
       aes(x = year, y = percent, fill = br)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~type) +
  labs(title = "Sales comparison by bedrooms: New Construction versus Resale", fill = "Bedrooms") +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
...
sf_permits_line <- ggplot(pha_cbps,
...
The high cost of homeownership and lifestyle preference changes have led to decreasing homeownership rates across the nation. In the region, homeownership has been in a slow decline from 2010. As with most urban areas, the homeownership rate in the City of Richmond remains low at 44 percent, while in the counties, homeownership rates remain well above 60 percent. With fewer multifamily options in the rural and suburban areas of the region, this trend will no doubt continue.
...
ho_line <- ggplot(pha_homeownership,
       aes(x = year, 
           y = ho_rate, 
           color = county)) +
...
Growing homeownership costs coupled with increasing debt from things like student and auto loans are having significant impact on younger households entering the homeownership market. The homeownership rate among householders under 25 years of age has remained below 10 percent from 2016 to 2020. Nearly three in four householders 45 years old or older are likely to be a homeowner, while half of householders between 25 and 44 years old are homeowners. These homeownership rates have remained largely unchanged throughout the latter half of the last decade. 
...
ggplotly(ho_age_line) |> 
  plotly_camera()
  
```
Across the region, the homeownership gap remains wide between white households and households of color. The gap is widest among white and Hispanic households, where the gap is nearly 30 percentage points in every locality other than Hanover County, where it is still wide at 24 percentage points.

The Black-white homeownership gap remains wide, as well --- 20 percentages points in every locality except for Hanover County, where the gap has been closing in recent years. The greatest disparities continue to be in Henrico County and the City of Richmond, where white homeownership remains comparatively higher than all other racial and ethnic groups.
```{r ho-rate-race}
