...
```
This chapter is an analysis of existing housing costs versus the ability of households in the region to afford those housing costs.
...
int_rates_plot <- ggplot(costs_income_afford,
                         aes(x = year,
                             y = annual_int)) +
  geom_line(size = 1, color = "#be451c") +
  geom_point_interactive(aes(data_id = annual_int,
                             tooltip = label_percent()(annual_int)),
                             size = 0.5,
                             color = "#be451c") +
  theme_pha() +
  scale_y_continuous(labels = label_percent(accuracy = 0.01),
                     ...
                     n.breaks = 6) +
  scale_x_continuous(n.breaks = 7) +
  labs(title = "Mortgage interest rates on the rise",
       subtitle ="Average annual rates for 30-year fixed-rate mortgages",
       caption = "**Source:** Freddie Mac, Primary Mortgage Market Survey.<br>**Note:** 2022 through August.") +
  add_zero_line(axis = "y")

if (knitr::is_html_output()) {
  
  girafe(ggobj = int_rates_plot,
         height_svg = 4) 
...
```{r}
#| label: fig-occ-rent
#| fig.cap: "Affordable rent by occupation versus average rents"

job_rent <- ggplot(occ_locality_afford,
                   aes(y = reorder(group, rent, decreasing = F),
                       x = rent,
                       fill = type,
                       data_id = rent,
  ...
  labs(title = "Most workers cannot afford rent on their own",
       subtitle = "Affordable rent by occupation versus average rents",
       caption = "**Sources:** U.S. Bureau of Labor Statistics, Occupational Employment and Wage Statistics; CoStar Group, Inc.") +
  theme_pha() +
...
job_own <- ggplot(occ_locality_afford,
  ...
  labs(title = "Most workers cannot buy a home on their own",
       subtitle = "Maximum affordable home price by occupation versus median sales prices",
       caption = "**Sources:** U.S. Bureau of Labor Statistics, Occupational Employment and Wage Statistics; CVR MLS.") +
  theme_pha() +
  add_zero_line(axis = "x") +
  flip_gridlines() +
  theme(strip.text.y = element_blank())
