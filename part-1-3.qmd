...
hh_inc_plot <- ggplot(b25118_data, 
       aes(y = income,
           x = change,
           fill = tenure,
           data_id = change,
...
med_inc_plot <- ggplot(b25119_cpi, 
       aes(x = year,
           y = dollars20,
           color = NAME)) +
  geom_line(stat = "identity", size = 1) +
  geom_point_interactive(
    aes(data_id = dollars20,
        tooltip = label_dollar()(dollars20),
        group = 1),
    size = 0.5) +
  facet_wrap(~tenure) +
  scale_color_pha() +
...
When comparing dollar figures at different points in time, it is important to account for the effects of [inflation](https://www.investopedia.com/ask/answers/042415/what-impact-does-inflation-have-time-value-money.asp) on the dollar value. The value of a dollar in 2020 is not the same as in 2010 because inflation increases 

To accurately compare data like median household incomes across time, we must convert all dollar values to the same timeframe. The [Consumer Price Index (CPI)](https://www.investopedia.com/terms/c/consumerpriceindex.asp) is used to adjust previous dollar figures to the most recent data point. 

:::
### Incomes by race and ethnicity

Typical incomes in the region remain unequal by race and ethnicity. Households with the highest incomes include Asian and white, non-Hispanic residents in the counties---earning well above \$75,000. Black and Hispanic households consistently have the lowest median incomes, along with multiracial households in Henrico and Richmond.
...
inc_race_plot <- ggplot(b19013_data,
       aes(x = year,
           y = dollars20/1000,
           color = race)) +
  geom_line(stat = "identity", size = 1) +
  ...
  labs(title = "Median household income by race and ethnicity",
       subtitle = "2016 to 2020 | 2020 inflation-adjusted dollars",
       caption = "**Note:** Estimates with low reliability are omitted.<br>**Source:** U.S. Census Bureau, American Community Survey, Table B19013.") +
  theme_pha() +
...
inc_child_plot <- ggplot(b19125_b19202_cpi,
       aes(x = year,
           y = dollars20/1000,
           color = type)) +
  facet_wrap(~NAME, nrow = 1) +
...
wage_occ_plot <- ggplot(oews_occ,
  ...
  geom_col() +
  geom_col_interactive(size = 2) +
  scale_x_continuous(labels = label_percent(), limits = c(-0.005, 0.23)) +
  scale_y_discrete(labels = function(x) str_wrap(x, width = 30)) +
  scale_fill_pha() +
  labs(title = "Percent change in annual wage for<br>top 5 most common occupation categories",
       subtitle = "Richmond, VA MSA | May 2019 to May 2021",
       caption = "**Source:** U.S. Bureau of Labor Statistics, Occupational Employment and Wage Statistics.") +
  theme_pha() +
  add_zero_line(axis = "x") +
