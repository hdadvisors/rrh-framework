...
cva_pop <- read_rds("data/pha_pop.rds") |> 
  filter(NAMELSAD == "Chesterfield County") |> 
  st_drop_geometry()

cva_pop_bar <- ggplot(cva_pop,
                      aes(x = year,
                          y = value)) +
  geom_col()

ggplotly(cva_pop_bar) |> 
  plotly_camera()
...
cva_change <- read_rds("data/pep_change_clean.rds") |> 
  filter(GEOID == cva)

pep_2021 <- read_rds("data/pep_2021.rds") |> 
  filter(GEOID == cva)

cva_change_21 <- rbind(cva_change, pep_2021)
...
cva_price <- read_csv("data/chesterfield_med_sales.csv") |> 
  clean_names() |> 
...
cva_price$month <- match(cva_price$month, month.abb)
...
cva_sub_rent <- read_csv("data/rr_submarket_data.csv") |> 
  clean_names() |> 
...
csub_rent_line <- ggplot(cva_sub_rent,
       aes(x = period,
           y = rent, 
           color = submarket)) +
  geom_line(stat = "identity") + 
...
cva_income <- b25119_cpi |> 
  filter(locality == "Chesterfield County") |> 
  filter(tenure == "Renter")

cva_rent <- read_csv("data/rr_annual_rent.csv") |> 
...
cva_rent_gap <- cva_income |> 
  left_join(cva_rent, by = "year") |> 
...
cva_sales_annual <- read_csv("data/rr_annual_sales.csv") |> 
