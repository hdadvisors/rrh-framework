...
```{r cb-income-acs-data, eval=FALSE}
...
```{r cb-hh, fig.cap="Change in cost-burdened households by household type"}
...
```{r mort-del, fig.cap="Mortgage delinquency rate by locality"}
...
```{r evictions, fig.cap="Evictions filings and judgements by locality"}
...
```{r evict-avg, fig.cap="Average annual eviction filings and orders by locality"}
...
```{r call-vol, fig.cap="Housing Resource Line monthly call volume"}
...
```{r call-topic, fig.cap="Housing Resource Line volume by call topic"}
...
```{r pit-plot, fig.cap="Greater Richmond CoC Point-in-Time count"}
