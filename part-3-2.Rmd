...
```{r cb-locality, fig.cap= "Percent cost-burdened by locality}
...
gg_bar_col_facet(cb_hh_pha,
             x_var = household_type,
                 y_var = difference,
                 facet_var = county,
             col_var = household_type) +
  labs(title = "Cost burden from 2015 to 2018") +
...
full_set_longer <- full_set_longer |> 
  filter(str_detect(co_c_name, "Richmond")) |> 
  filter(str_detect(category, "overall"))
