...
```{r ind-living, fig.cap=paste0(name, ": Net change in individuals with independent living difficulties by age")}
...
```{r noah, fig.cap=paste0(name, ": Distribution of average asking rents in NOAH properties by year built")}
...
```{r income-rent-gap, fig.cap=paste0(name, ": Average asking rent versus rent affordable to median renter income")}
...
```{r income-own-gap, fig.cap=paste0(name, ": Income needed to afford median home price versus median renter income")}
...
```{r gap, eval=FALSE, fig.cap=paste0(name, ": Rental housing gap by AMI"), include=FALSE}
